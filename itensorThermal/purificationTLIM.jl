...
function ITensors.op(::OpName"expτTL", ::SiteType"S=1/2", s1::Index, s2::Index; τ)
  gx = 0.5
  gz = 0.5
  h = op("Z", s1) * op("Z" , s2) +
      gx / 2 * op("X", s1) * op("Id", s2) +
      gx / 2 * op("Id", s1) * op("X", s2)
      gz / 2 * op("Y", s1) * op("Id", s2) +
      gz / 2 * op("Id", s1) * op("Y", s2)
      # g * op("X", s1) * op("Id", s2) +
      # g * op("Id", s1) * op("X", s2)
  return exp(τ * h)
end
...
function main(; N=10, cutoff=1E-8, δτ=0.1, beta_max=2.0)
  ...
  @printf("Making gates...\n")
  gates = ITensors.ops([("expτTL", (n, n + 1), (τ=-δτ / 2,)) for n in 1:(N - 1)], s)
