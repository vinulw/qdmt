...
def testFirstOrderTrotterEvolveTransposed():
    d = 2
    D = 4
    U1 = unitary_group.rvs(d**2)
    U1ten = U1.reshape(d, d, d, d)
    ...
    evolvedRho6 = IUUI.conj() @ UUU.conj() @ rho6 @ UUU.T @ IUUI.T

    evolvedRho6 = evolvedRho6.reshape([d]*(2*N))
    evolvedRho6 = ncon([evolvedRho6,], ((1, -1, -2, -3, -4, 2, 1, -5, -6, -7, -8, 2),))

    evolvedTen = firstOrderTrotterEvolveTransposed(A, U1, U2, N=4, l=l, r=r)

    assert np.allclose(evolvedRho6, evolvedTen)

    # Check if one evolution is the transpose of the other
    evolvedTenOld = firstOrderTrotterEvolve(A, U1, U2, N=4, l=l, r=r)

    evolvedTenOld = evolvedTenOld.transpose(4, 5, 6, 7, 0, 1, 2, 3)

    assert np.allclose(evolvedTenOld, evolvedTen)

if __name__=="__main__":
    testFirstOrderTrotterEvolveTransposed()
