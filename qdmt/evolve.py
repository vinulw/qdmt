...
def firstOrderTrotterEvolveTransposed(A, U1, U2, N, l=None, r=None):
    '''
    Evolve density matrix represented by $A$ using a first order trotter
    evolution.
    ...
    # State tensor contractions
    ADagContr = genAContr(1, 4, 7, 6, N+2)
    AContr = genAContr(2, 5, 8, 6, N+2)

    # Get U contr
    evenADagContr = genUContr(3, 9, 4, 10, 12, 12, N//2 + 1)
    oddADagContr = genUContr(-1, -2, 9, 15, -2, 12, N//2)

    # Get UDagContr
    evenAContr = genUContr(6, 12, 5, 11, 12, 12, N//2 + 1)
    evenAContr[0][0] = evenADagContr[0][0]
    evenAContr[-1][1] = evenADagContr[-1][1]
    oddAContr = genUContr(-1-N, -2-N, 12, 18, -2, 12, N//2)
    # l/r contr
    lcontr = [ADagContr[0][0], AContr[0][0]]
    rcontr = [AContr[-1][2], ADagContr[-1][2]]
